...
import { Wallet, Plus, ArrowUpRight, ArrowDownLeft, PieChart, Users, TrendingUp, AlertCircle, Loader2, WifiOff, Copy, Check, Trophy, CheckCircle2, Share2 } from 'lucide-react';
...
function ExpensesContent() {
    ...
    const [summary, setSummary] = useState<any[]>([]); // settlements array
    const [stats, setStats] = useState<any>(null); // total spent etc.
    ...
    const fetchGroupData = useCallback(async (id: string) => {
        try {
            if (isOnline()) {
                ...
                setSummary(settlements);
                setGroups(groupsRes.data);

                // Calculate totalSpent from loaded expenses (EXCLUDING Settlements)
                const total = loadedExpenses
                    .filter((exp: any) => exp.type !== 'Settlement')
                    .reduce((sum: number, exp: any) => sum + (Number(exp.amount) || 0), 0);
                const statsData = { totalSpent: total };
                setStats(statsData);

                saveToCache(`expenses_${id}`, loadedExpenses);
                saveToCache(`stats_${id}`, statsData);
                saveToCache(`settlements_${id}`, settlements);
                saveToCache('groups', groupsRes.data);

                const updatedGroup = groupsRes.data.find((g: any) => g._id === id);
                if (updatedGroup) {
                    setSelectedGroup(updatedGroup);
                    // If group is finished, fetch summary data for the modal
                    if (updatedGroup.status === 'finished') {
                        try {
                            const summaryRes = await api.get(`/groups/${id}/summary`);
                            setWrappedData(summaryRes.data);
                            if (summaryRes.data.badges) {
                                setNewBadges(summaryRes.data.badges);
                            }
                        } catch (e) {
                ...
                // Re-calculate stats and settlements locally
                const localSettlements = calculateSettlements(allExpenses as any);
                setSummary(localSettlements);

                const total = allExpenses
                    .filter((exp: any) => exp.type !== 'Settlement')
                    .reduce((sum, exp) => sum + (Number(exp.amount) || 0), 0);
                setStats({ totalSpent: total });
            }
        } catch (err: any) {
    ...
    const handleFinishTour = async () => {
        ...
        if (!confirm('Are you sure you want to finalize this tour? This will mark it as finished and award badges to all members.')) {
            return;
        }

        setIsFinishing(true);
        try {
            const response = await api.post(`/groups/${selectedGroup._id}/finish`);
            setWrappedData(response.data.summary);

            // Extract my new badges from the response
            const userData = localStorage.getItem('user');
            if (userData) {
                const user = JSON.parse(userData);
                const myBadges = response.data.newBadges.find((b: any) => b.userId === user.id)?.badges || [];
                setNewBadges(myBadges);
            }

            setIsWrappedModalOpen(true);
            fetchGroupData(selectedGroup._id); // Refresh status
        } catch (err: any) {
            alert('Failed to finish tour: ' + (err.response?.data?.message || err.message));
        } finally {
            ...
            try {
                ...
                if (cachedGroups.length > 0) {
                    ...
                    if (groupId) {
                        ...
                        const cachedStats = getFromCache(`stats_${groupId}`);
                        const cachedSettling = getFromCache(`settlements_${groupId}`);
                        if (cachedExp) setExpenses(cachedExp);
                        if (cachedStats) setStats(cachedStats);
                        if (cachedSettling) setSummary(cachedSettling);
    ...
    const onSubmit = async (data: ExpenseFormValues) => {
        ...
        if (isOnline()) {
            ...
            const localSettlements = calculateSettlements(updatedExpenses as any);
            setSummary(localSettlements);
            saveToCache(`settlements_${selectedGroup._id}`, localSettlements);

            const total = updatedExpenses
                .filter(exp => exp.type !== 'Settlement')
                .reduce((sum, exp) => sum + (Number(exp.amount) || 0), 0);
            setStats({ totalSpent: total });
            saveToCache(`stats_${selectedGroup._id}`, { totalSpent: total });
        ...
        <div className="space-y-8 md:space-y-10">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto items-center">
                    {selectedGroup && !selectedGroup.coverImage && (
                        <Button
                            onClick={() => setIsMemoryModalOpen(true)}
                            variant="outline"
                            className="h-14 bg-white/5 border-white/10 hover:bg-white/10 text-white rounded-2xl px-6 font-black uppercase tracking-widest text-[10px]"
                        >
                            <Camera className="mr-2 w-5 h-5 text-emerald-400" />
                            {t('memory')}
                        </Button>
                    )}
                    <select
                    ...
                    <div className="flex gap-2 w-full sm:w-auto">
                        ...
                        {/* Scan Button (Restored original style) */}
                        <div className="relative group flex-grow md:flex-grow-0">
                            ...
                            <Button
                                ...
                                disabled={isScanning}
                                className="w-full sm:w-auto rounded-2xl h-14 px-6 bg-blue-600/10 text-blue-500 border border-blue-500/20 hover:bg-blue-600 hover:text-white transition-all shadow-xl shadow-blue-500/10 font-black uppercase tracking-widest text-[10px]"
                                size="lg"
                            >
                                {isScanning ? (
            ...
            {selectedGroup && (
                <motion.div
                    ...
                    className="relative w-full h-[300px] md:h-[400px] rounded-[2rem] md:rounded-[3rem] overflow-hidden group shadow-2xl bg-white/[0.02] border border-white/[0.05]"
                >
                    {selectedGroup?.coverImage ? (
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-[radial-gradient(circle_at_center,rgba(16,185,129,0.05)_0%,transparent_100%)]">
                            <Camera className="w-12 h-12 text-zinc-800 mb-4" />
                            <p className="text-zinc-600 text-[10px] font-black uppercase tracking-widest font-black uppercase tracking-widest leading-none mb-1">No Memory Captured Yet</p>
                        </div>
                    )}
                    ...
                    <div className="absolute top-6 right-6 md:top-10 md:right-10 z-20">
                        <Button
                            onClick={() => setIsMemoryModalOpen(true)}
                            className="rounded-2xl h-12 px-6 bg-white/5 backdrop-blur-xl border border-white/10 text-white hover:bg-white/10 shadow-2xl transition-all font-black uppercase tracking-widest text-[10px]"
                        >
                            <Camera className="mr-2 w-4 h-4 text-emerald-400" />
                            {selectedGroup?.coverImage ? t('memory') : 'Capture Memory'}
                        </Button>
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="glass p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border-white/10 bg-gradient-to-br from-white/[0.05] to-transparent col-span-1 sm:col-span-2 relative overflow-hidden group">
                    <div className="relative z-10">
                        ...
                        <h3 className="text-4xl md:text-6xl font-black tracking-tighter transition-all">৳ {stats?.totalSpent?.toLocaleString() || stats?.totalSpent || 0}</h3>
            ...
            {/* Finish Tour Secondary Section (Bottom) */}
            {selectedGroup && (
                <div className="pt-20 border-t border-white/5 flex flex-col items-center justify-center gap-8 text-center pb-20 mt-10">
                    <div className="space-y-3">
                        <div className="w-16 h-16 rounded-full bg-zinc-900 border border-white/5 flex items-center justify-center mx-auto mb-4">
                            <Trophy className={`w-8 h-8 ${selectedGroup.status === 'active' ? 'text-zinc-700' : 'text-emerald-500'}`} />
                        </div>
                        <h4 className="text-2xl font-black text-white italic uppercase tracking-tighter">
                            {selectedGroup.status === 'active' ? 'Adventure Ongoing' : 'Adventure Completed'}
                        </h4>
                        <p className="text-zinc-500 text-sm max-w-sm mx-auto font-medium">
                            {selectedGroup.status === 'active'
                                ? 'Ready to wrap up? Finalize this tour once all costs are logged. This will generate your trip summary and award squad badges!'
                                : 'This legendary journey has been archived. Check out your trip wrapped and badges below.'}
                        </p>
                    </div>
                    ...
                    {selectedGroup.status === 'active' ? (
                        <div className="flex flex-col items-center gap-4">
                            <Button
                                ...
                                className="bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded-[2.5rem] h-20 px-12 uppercase tracking-widest text-sm gap-4 transition-all shadow-2xl shadow-emerald-500/20 active:scale-95"
                            >
                                {isFinishing ? (
                            ...
                            <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-[0.2em]">Group Admin Action Only</p>
                        </div>
                    ) : (
                        <Button
                            onClick={() => setIsWrappedModalOpen(true)}
                            className="bg-blue-600 hover:bg-blue-700 text-white font-black rounded-[2.5rem] h-20 px-12 uppercase tracking-widest text-sm gap-4 shadow-2xl shadow-blue-500/20 active:scale-95 transition-all"
                        >
